...
int check_d(char **grid, int y, int x, int add_x)
{
	int new_y = y - 1;
	int new_x = x + add_x;
	...
	if (grid[new_y][new_x] == '1')
		return(0);
	return(check_d(grid, new_y, new_x, add_x));
}
...
int is_valid(char **grid, int y, int x)
{
	...
	return(check_d(grid, y, x, -1) && check_d(grid, y, x, 1));
}
